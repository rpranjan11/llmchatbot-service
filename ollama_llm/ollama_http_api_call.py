...
def get_ollama_generic_response(model, prompt):
    try:
        ...
        response = httpx.get(f'http://61.32.218.74:8930/ollamagenericresponse/{model}/{prompt}')
...
def get_ollama_file_response(model, prompt):
    try:
        logging.debug(f"Requesting Ollama API with model: {model}, prompt: {prompt}")
        response = httpx.get(f'http://61.32.218.74:8930/ollamacustomresponse/{model}/{prompt}')
        response.raise_for_status()  # Raise an exception for HTTP errors
        logging.debug(f"Response: {response.json()}")
        return response.json()
    except httpx.RequestError as exc:
